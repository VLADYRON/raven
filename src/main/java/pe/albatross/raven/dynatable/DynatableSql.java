package pe.albatross.raven.dynatable;
...
import pe.albatross.raven.Octavia;
...
public class DynatableSql {
    ...
    public List all(EntityManager entityManager) {
        ...
        filter.setTotal(((Long) this.totalQuery.find(entityManager)).intValue());
        filter.setFiltered(((Long) this.filteredQuery.find(entityManager)).intValue());

        this.dataQuery.limit(filter.getOffset(), filter.getPerPage());

        return this.dataQuery.all(entityManager);
    }
